...
//   float t = clamp(temp, 1.0, 40.0);
//   vec3 color;
//   if (t <= 6.6) { color.r = 1.0; } else { color.r = 1.292936 * pow(t - 6.0, -0.1332047592); }
//   if (t <= 6.6) { color.g = 0.39008 * log(t) - 0.63184; } else { color.g = 1.129890 * pow(t - 6.0, -0.0755148492); }
//   if (t >= 6.6) { color.b = 1.0; } else if (t <= 1.9) { color.b = 0.0; } else { color.b = 0.54320 * log(t - 1.0) - 1.19625; }
//   color = clamp(color, 0.0, 1.0);
//   color *= vec3(1.0, 0.85, 0.55);
//   return color;
// }
...
//   if (r < uDiskInner || r > uDiskOuter) discard;
//   float diskFraction = (r - uDiskInner) / (uDiskOuter - uDiskInner);
//   float tempK = mix(18.0, 3.0, pow(diskFraction, 0.4));
...
//   float totalShift = doppler * gravRedshift;
//   float shiftedTemp = tempK * totalShift;
//   vec3 color = blackbody(shiftedTemp);
//   float brightness = pow(clamp(totalShift, 0.1, 5.0), 3.5);
//   float rNorm = r / uDiskInner;
//   float radialBrightness = 1.0 / pow(rNorm, 2.5);
//   float ringPattern = 1.0 + 0.3 * sin(r * 25.0) + 0.15 * sin(r * 60.0 + 1.0);
...
//   float outerFade = smoothstep(0.0, 0.35, 1.0 - diskFraction);
//   float edgeFade = innerFade * outerFade;
//   float thickness = 0.02 + 0.015 * (1.0 - diskFraction);
//   float yFade = exp(-abs(vWorldPos.y) / thickness);
//   color *= brightness * radialBrightness * turbulence * edgeFade * yFade * 12.0;
//   float alpha = clamp(length(color) * 2.0, 0.0, 1.0) * edgeFade * yFade;
